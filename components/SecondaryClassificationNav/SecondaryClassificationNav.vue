<template>
    <view class="gui-grid gui-dark-bg-level-3 gui-padding">
        <view v-for="(item, index) in navData" :key="index" @click="handleNavItemOnClick(item)" class="gui-grid-item gui-m-10 grid4 gui-bg-black-opacity1">
            <text class="gui-grid-text" style="color: #fff">{{ item.name }}</text>
        </view>
        <view class="gui-grid-item gui-m-10 grid4 gui-bg-black-opacity1" @click="goToCategoryPage">
            <text class="gui-grid-text" style="color: #fff">全部</text>
        </view>
    </view>
</template>
