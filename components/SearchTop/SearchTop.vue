    ...
    <view>
        ...
        <view class="search-container">
            ...
            <view class="search-input-container" v-if="isNeedSearchInput">
                <SearchInput ref="searchInputRef" class="gui-flex1 search-input" placeholder="请输入" :kwd="props.queryParam.keyword" @handleBlur="handleBlur" @clear="handleClear" @inputting="handleInputting" @confirm="handleConfirm"></SearchInput>
                <button @click="handleConfirm" type="default" class="gui-button gui-button-mini gui-bg-yellow gui-noborder" style="width:100rpx;">
                    <text class="gui-color-white gui-button-text-mini gui-icons">搜索</text>
                </button>
            </view>
            <!--    搜索建议-->
            <view class="search-suggestion-container gui-padding-x" v-if="searchSuggestionsList.length">
                <scroll-view :show-scrollbar="false" :scroll-y="true" style="max-height: 500rpx" :scroll-top="0">
                    <view class="search-suggestions-item" v-for="(item,index) in searchSuggestionsList" @click="handleSearchSuggestItemOnClick(item)" :key="index">
        ...
        <view class="search-history-container gui-padding" v-if="!queryParam.keyword.trim() && isNeedSearchInput">
            <view class="search-history-title gui-flex gui-rows gui-nowrap gui-align-items-center" style="color: #fff">
                <text class="gui-h6 gui-flex1 gui-bold">搜索历史</text>
                <text @click="handleClearSearchHistory" class="gui-icons">&#xe794;</text>
            </view>
            <view class="search-history-item-container gui-flex gui-rows gui-wrap" style="color: #fff">
                <view v-for="(item,index) in searchHistoryList" :key="item+index" @click="handleSearchSuggestItemOnClick(item)" class="search-history-item gui-color-grey1 gui-text-small">{{ item }}</view>
            </view>
        </view>
        <!--  筛选条件-->
...
watch(
  ...
  (newVal) => {
    ...
    let attributeListTarget =
      menuList.value.find((item) => item.type === "filter" && item.children?.length)?.children || []
    // 过滤掉未选择的
...
onMounted(() => {
  // 初始化从本地取出搜索历史
  searchHistoryList.value = uni.getStorageSync("searchHistoryList")
    ? JSON.parse(uni.getStorageSync("searchHistoryList"))
    : []
  // category1Id存在在请求
