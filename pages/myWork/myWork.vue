...
<template>
    <view class="content">
        <z-paging ref="zPagingRef" v-model="(pageData as any)[pageData.currentPageNav].list" @query="getListInfo" show-refresher-update-time auto-show-back-to-top>
            ...
            <!--			专辑列表Item-->
            <AlbumItemCard v-if="pageData.currentPageNav === 'albumListInfo'" v-for="(item,index) in pageData.albumListInfo.list" :data="item" :editItemHandler="addOrEditItem" :deleteItemHandler="deleteItem" :key="item.albumId"></AlbumItemCard>
            <!--			声音列表Item-->
            <TrackItemCard v-if="pageData.currentPageNav === 'trackInfoListInfo'" v-for="(item,index) in pageData.trackInfoListInfo.list" :data="item" :editItemHandler="addOrEditItem" :deleteItemHandler="deleteItem" :key="item.trackId"></TrackItemCard>
        </z-paging>
    </view>
</template>
...
const deleteItem = (id: number | string) => {
  if (pageData.currentPageNav === "albumListInfo") {
    // 专辑列表
    pageData.albumListInfo.list = pageData.albumListInfo.list.filter(
      (item: AlbumInfoListInterface) => item.albumId !== id
    )
  } else if (pageData.currentPageNav === "trackInfoListInfo") {
    // 声音列表
    pageData.trackInfoListInfo.list = pageData.trackInfoListInfo.list.filter(
      (item: TrackInfoListInterface) => item.trackId !== id
    )
  }
}
// 添加
...
const select = (index: number) => {
  ...
  // 切换列表的状态信息
  ;(pageData as { [key: string]: any })[pageData.currentPageNav].status = (pageData as { [key: string]: any })[
    pageData.currentPageNav
