		...
		<template v-slot:gBody>
			<view class="gui-relative" style="background-color: pink;">
				...
				<view class="gui-absolute-lb gui-creation-live" style="background-color: #000">
					...
					<scroll-view scroll-y style="height: 620rpx; width:670rpx;" class="gui-p-10">
					  <view class="gui-flex gui-align-items-center gui-m-r-50 gui-m-b-20" v-for="item in orderInfo?.orderDetailList" :key="item.id">
					  	...
					  	<view class="gui-flex gui-column gui-flex1 gui-m-l-20">
					  		<text class="gui-block gui-ellipsis-line2 gui-h5 gui-color-white">{{ item?.itemName }} ￥{{ item?.itemPrice }}</text>
					  	</view>
					  </view>
					</scroll-view>

					<view class="gui-flex gui-space-between gui-align-items-end gui-m-t-10">
						<text class="gui-color-white">商品总价</text>
					...
					<view class="gui-flex gui-space-between gui-align-items-end gui-m-t-10">
						<text class="gui-color-white">实付款</text>
						<text class="gui-block gui-color-white">
					...
					<view class="gui-flex gui-space-between gui-align-items-end gui-m-t-10">
						<view class="gui-color-white">订单号</view>
						<br>
						<view class="gui-block gui-color-white">
							<text>{{ orderInfo?.orderNo }}</text>
						</view>
					</view>
					<view class="gui-border-b gui-m-t-20 gui-m-b-20"></view>
					<view class="gui-flex gui-space-between gui-align-items-end gui-m-t-10">
						<text class="gui-color-white">付款方式</text>
						<text class="gui-block gui-color-white">
